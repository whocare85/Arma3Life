...
A3L_TFEnabled = true;
A3L_onTsServer = "Arma 3 Life Teamspeak Server" == (call TFAR_fnc_getTeamSpeakServerName);
...
[] spawn {
	...
	while {true} do {
				...
				if (!(_isadmin)) then {
					...
					_onTsServer = "Arma 3 Life Teamspeak Server" == (call TFAR_fnc_getTeamSpeakServerName);
					if (!(_onTsServer)) then {
						titleText ["Please join the ArmA 3 Life teamspeak server!", "BLACK"];
